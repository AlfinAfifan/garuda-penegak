...
export const DELETE = async (req: Request, { params }: { params: Promise<{ id: string }> }) => {
  try {
    ...
    if (existingData.laksana) {
      return new NextResponse('Cannot delete Bantara data when Laksana is present', { status: 400 });
    }

    const deletedData = await Tku.findByIdAndUpdate(id, { bantara: false, sk_bantara: '', date_bantara: null });
