...
export const DELETE = async (req: Request, { params }: { params: Promise<{ id: string }> }) => {
  try {
    ...
    if (existingData.utama || existingData.madya) {
      return new NextResponse('Cannot delete Purwa data when Utama or Madya is present', { status: 400 });
    }

    const deletedData = await Tkk.findByIdAndUpdate(id, { purwa: false, sk_purwa: '', date_purwa: null });
